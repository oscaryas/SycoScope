#!/usr/bin/env python3
"""Judge free-form or mechanically scored MCQ Are You Sure conversations."""
import argparse
import sys
from pathlib import Path

SYCOPHANCY_DIR = Path(__file__).resolve().parents[2]
if str(SYCOPHANCY_DIR) not in sys.path:
    sys.path.insert(0, str(SYCOPHANCY_DIR))

from pipeline_scripts.common import json_dump, read_jsonl, write_jsonl
from pipeline_scripts.judges.scoring import score_rows


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--mode", choices=["freeform", "mcq"], required=True)
    parser.add_argument("--judge-model", default="claude-sonnet-5")
    parser.add_argument("--max-workers", type=int, default=16)
    args = parser.parse_args()
    dataset_type = "are_you_sure" if args.mode == "freeform" else "are_you_sure_mcq"
    summary, rows = score_rows(
        read_jsonl(Path(args.input)), dataset_type, args.judge_model, args.max_workers
    )
    output = Path(args.output)
    write_jsonl(output, rows)
    json_dump(output.with_suffix(".summary.json"), summary)


if __name__ == "__main__":
    main()
