...
See ../SYCONBENCH_EVAL.md for generation, extraction, scoring, and caveats.
"""
import argparse
...
def main():
    ...
    from analyze_probes import eval_common
    ...
    probe_meta = json.loads((run_dir / "activations/meta.json").read_text())
    if canonical_model(probe_meta["model"]) != canonical_model(args.model):
        raise ValueError("extraction model does not match the probe-training model")
    ...
    out_dir = run_dir / "eval_syconbench"
    out_dir.mkdir(parents=True, exist_ok=True)
    prep_path = out_dir / "preparation.json"
    if prep_path.exists():
        previous = json.loads(prep_path.read_text())
        if previous["records_sha256"] != fingerprint:
            raise ValueError("prepared data differ; use a separate run/cache rather than overwrite")
    ...
    if not (out_dir / "activations.npz").exists():
        fields = (*LABEL_FIELDS, "group_id", "turn", "prompt_condition")
        eval_common.extract_activations(out_dir, records, fields, args)
        meta_path = out_dir / "meta.json"
        meta = json.loads(meta_path.read_text())
        meta.update(records_sha256=fingerprint, evaluation_selection={"frac": args.selection_frac, "seed": args.seed})
        meta_path.write_text(json.dumps(meta, indent=2), encoding="utf-8")
    meta = json.loads((out_dir / "meta.json").read_text())
    if meta.get("records_sha256") != fingerprint or canonical_model(meta["model"]) != canonical_model(args.model):
        raise ValueError("cached model/data provenance mismatch")
    if meta.get("evaluation_selection") != {"frac": args.selection_frac, "seed": args.seed}:
    ...
    scores = eval_common.score_all(run_dir, out_dir, slugs, positions, layers, LABEL_FIELDS, selection, args.seed)
    eval_common.report(out_dir, scores, positions, layers, LABEL_FIELDS, slugs, {
        "n_records": len(index), "selection_frac": args.selection_frac, "seed": args.seed,
        "note": "Separate setting/turn-failure targets; question-grouped split. Response pooling covers current answer only.",
    })
