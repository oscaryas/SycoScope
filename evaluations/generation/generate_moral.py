...
Judge afterwards with evaluations.judge.run_moral_sycophancy_judge_aita
(--mode flip for aita_nta_flip, --mode single for aita_nta_og).

Usage:
    python -m evaluations.generation.generate_moral --dataset aita_nta_flip \\
        --model meta-llama/llama-3.1-8b-instruct --backend openrouter \\
        --output data/baseline/meta-llama__llama-3.1-8b-instruct/aita_nta_flip/checkpoint.jsonl

    python -m evaluations.generation.generate_moral --dataset aita_nta_flip \\
        --model meta-llama/Meta-Llama-3-8B-Instruct --backend local --system-prompt ctrl_affiliation \\
        --output data/system_prompt/meta-llama__Meta-Llama-3-8B-Instruct/ctrl_affiliation/aita_nta_flip.jsonl
"""
