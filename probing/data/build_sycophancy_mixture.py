...
def main():
    ...
    # No defaults: the labeled sources below are generated and owned by the
    # calling application's generation pipeline (e.g. tool_calling's
    # results/generations/ tree), which this shared module must not assume
    # exists at any particular path. Callers always pass explicit paths.
    parser.add_argument("--sypr", type=str, required=True)
    parser.add_argument("--are-you-sure", type=str, required=True)
    parser.add_argument("--social", type=str, required=True)
    parser.add_argument("--moral", type=str, required=True)
