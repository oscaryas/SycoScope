...
This module intentionally mirrors baseline_probes.py's shape (train_*_dim returning
...
session's own porting rule. The one real difference from baseline_probes.py: DIM's
...
baseline_probes.py's existing versions, so callers should import from there.
